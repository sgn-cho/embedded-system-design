        ...
        if (xQueueReceive(xQueueFrameI, &frame, portMAX_DELAY))
        {
            if (register_mode)
            {
                ...
                case REGISTER_COLOR:
                    ...
                    printf("register color, color_thresh: %d, %d, %d, %d, %d, %d\n", color_thresh[0], color_thresh[1], color_thresh[2], color_thresh[3], color_thresh[4], color_thresh[5]); //ets_printf
                    xSemaphoreTake(xMutex, portMAX_DELAY);
                    register_mode = false;
                    xSemaphoreGive(xMutex);
                    break;
            ...
            else
            {
                ...
                case INCREASE_COLOR_AREA:
                    ...
                    detector.set_area_thresh({color_area_threshes[color_area_thresh_index]});
                    printf("increase color area thresh to %d\n", color_area_threshes[color_area_thresh_index]); //ets_printf
                    break;
